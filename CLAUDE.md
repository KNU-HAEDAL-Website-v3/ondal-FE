...
- HOJ는 같은 앱 안의 **모드**(2026-09-19 PM, docs 결정 9 - 별도 앱 분리(결정 8)는 철회): `/problems/*`·`/admin/tags` 는 `HojShell`(상단 가로 메뉴: 문제 · 채점 현황 · 랭킹 · 내 페이지 · 태그 관리(관리자)), 나머지는 `AppShell`(사이드바). 서로 오가는 버튼은 `AppSwitchButton` - "~로 이동할까요?" 확인 뒤 이동. HOJ 로 갈 때는 늘 문제 목록(2026-09-20 PM), Ondal 로 돌아올 때는 마지막에 보던 화면(`lib/appSwitch` `entryPath`). 별도 빌드·도메인·환경 변수 없음
...
- HOJ 지원(2026-09-19, BE V9): 문제에 **난이도**(`difficulty` 1~25, 표기 "대분류-소분류" `lib/difficulty` · `DifficultyBadge`)와 **허용 언어**(`allowedLanguages`, `lib/languages`)가 있다. 목록은 난이도 열·대분류 필터, 출제 폼은 대분류/소분류 셀렉트 + 언어 칩, 제출 폼(HOJ 연습·과제 `SubmissionForm`)은 허용 언어로 선택지를 좁힌다(서버도 400). 본문은 **마크다운**(`MarkdownView`, react-markdown + remark-gfm, HTML 미렌더). 관리자 "문제 가져오기"(`ImportProblemsDialog`)는 **깃허브에서 가져오기**가 기본 - `GET/POST /api/problems/import/github`, 서버가 HOJ 레포(ondal-problems) main 을 직접 읽음(`useProblemBankSource`·`useImportFromGithub`, configured=false 면 버튼 비활성 + 안내). 번들 JSON 파일 업로드(`POST /api/problems/import`)는 보조 경로 - `api/problemImport` 가 번호순 **10문제씩 나눠 순차 전송**(묶음마다 서버 트랜잭션, 도중 실패는 `ImportChunkError` 로 어디까지 저장됐는지 안내 - 같은 파일 재업로드로 이어짐). 과제 상세의 자동 채점 안내(`JudgeSamplesSection`)도 허용 언어만 표시
...
- HOJ P3(2026-09-20, docs 결정 13 · 계약 `hoj/api.md`): HOJ 메뉴 5개(문제 · 채점 현황 `/problems/status` · 랭킹 `/problems/ranking` · 내 페이지 `/problems/users/{내 id}` · 태그 관리) - 정적 경로가 `:problemId` 보다 먼저. 목록 = 푼 사람·제출·정답률 열 + 내 상태 필터(`myStatus`·`bookmarked` 클라이언트 필터) + 북마크 별(`BookmarkButton`, PUT/DELETE 낙관적 갱신) + 랜덤 문제 / 상세 = 북마크 · "다른 사람 풀이"(`AcceptedSolutionsSection`, `myStatus==='SOLVED'||canEdit` 일 때만, 403 `NOT_SOLVED` 안내) · 운영진 "정답 코드 보기"(`SolutionsDialog`, `solutionLanguages` 비면 안내) · "내 입력으로 실행"(`RunPanel`, `POST /run`, 429·503 안내) · 언어별 템플릿(`lib/codeTemplates`) · 마지막 언어 기억(localStorage) · Ctrl+Enter 제출(`CodeEditor onSubmit`) · 내 기록 "다시 편집" / 출제·수정 폼 = 정답 코드 절(`SolutionsSection`, 저장은 문제 → 채점 설정 → `PUT /solutions`) / 채점 현황 `HojStatusPage`(연습 제출만, 판정·언어 서버 파라미터, 검색 클라이언트, 채점 중 5초 refetch, beforeId 커서 "더 보기") / 랭킹 `HojRankingPage`(푼 문제 수, 분반 필터, 내 순위 카드·내 행 강조) / 사용자 페이지 `HojUserPage`(KPI 4장·언어 비율·푼 문제 격자·활동 잔디 `ActivityHeatmap`·태그 숙련도·최근 제출 `SubmissionFeedTable`) / 마이페이지 = "HOJ 내 페이지" 링크 + 편집기 설정(글꼴 크기·탭 폭, `lib/editorSettings`). **정답률·순위 등 값은 전부 서버 응답 그대로**
...
- 프로필 사진·상단 바(2026-09-20, docs 결정 14): `UserAvatar`(사진 있으면 `<img>`, 없거나 실패면 이름 첫 글자) 를 `AppShell` 상단 바(사진·이름·드롭다운: 마이페이지·로그아웃 - 원안 ui-v1 상단 바 문법)·`HojShell`·마이페이지 헤더가 공유. 사진은 서버 `me.avatarUrl`(홈페이지 구글 프로필, ID 토큰 picture) - **업로드 화면 없음**
- 기준본: docs 레포의 와이어프레임 v2.1
- 용어: UI는 "분반"(내부 모델명 Cohort), "과제/문제"(내부 Assignment). **"문제 은행"이라는 표현은 쓰지 않는다 - 전부 HOJ** (PM 2026-09-20)
- 원안(docs `screens/ui-v1`) 전수 조사 반영(2026-09-20, docs `audit-fe-2026-09-20.md`):
  - 과제 상세·HOJ 문제 상세: 학생의 자동 채점 과제는 xl 이상 좌 문제·우 편집기 2단(`xl:sticky`), 편집기 도구 줄에 초기화·전체 화면(`FullscreenPane`)·테마, "임시 저장됨 HH:MM:SS" 표시, 파일 탭은 드래그 첨부, 헤더에 분반명 브레드크럼·남은 시간(`remainingLabel`)·이전/다음 과제
  - 운영진 과제 상세: 현황판(`StatusBoard`)이 설명 바로 아래 - 상태·판정 필터, 이름 검색, 범례, CSV, "총 N명". 본인 제출란·기록은 접힌 `<details>`. 헤더는 내 상태 배지 대신 "제출물 N건"
  - 운영진 홈: "오늘의 할 일"(최근 차시 출석 미확인 · 선택 과제 미제출) + 현황 표 판정 열. 관리자 홈은 맨 위에 `AdminOverview`(전체 부원·승인 대기·진행 중 분반·HOJ 문제, `GET /api/health`, 승인 대기 명단)
  - 셸: 사이드바 순서 홈·내 수업·과제·공지·출석·Q&A·관리, 운영진 "새 공지 작성" CTA, 상단 바 "Ondal › 현재 메뉴", HOJ 상단 바 아바타 → 마이페이지. 공지 목록은 필터 패널(대상·검색)·행 수정/삭제·"총 N건"
